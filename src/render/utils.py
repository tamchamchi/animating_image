...
def load_gif_frames(path, skip_frames=4, scale=(100, 100)):
    ...
    try:
        ...
        for i in range(0, num_frames, skip_frames):
            ...
            frame = pil_img.convert("RGBA").resize(scale, Image.Resampling.LANCZOS)
...
def create_spotlight_mask(radius, max_opacity=255):
    """
    Creates a radial gradient surface to act as a light source mask.
    Used to 'subtract' alpha from the darkness layer.

    Args:
        radius (int): The radius of the light circle.
        max_opacity (int): The alpha value at the center (amount to subtract).

    Returns:
        pygame.Surface: A surface containing the gradient "light".
    """
    # Create a surface capable of holding alpha values
    light_surf = pygame.Surface((radius * 2, radius * 2), pygame.SRCALPHA)

    # Draw concentric circles from outside in
    # This creates a soft gradient (quadratic falloff looks more natural than linear)
    for r in range(radius, 0, -2):
        # Calculate alpha: 255 at center, 0 at edge
        # Using power of 2 makes the edge softer (closer to Gaussian)
        alpha = int(max_opacity * (1 - (r / radius) ** 2))

        # Draw circle. The RGB color doesn't matter much for subtraction blending,
        # but the Alpha channel is critical.
