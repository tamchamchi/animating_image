...
class Player:
    ...
    def update(self, dx, is_dancing, platforms):
        ...
        for plat in platforms:
            if self.rect.colliderect(plat.rect):
                if dx > 0: self.rect.right = plat.rect.left
                elif dx < 0: self.rect.left = plat.rect.right
        ...
        # --- ANIMATION STATE ---
        if self.is_jumping: self.set_state("jump", 0.3)
        elif dx != 0: self.set_state("run", 0.6)
        elif is_dancing and self.on_ground: self.set_state("dance", 0.3)
        else: self.set_state("idle", 0.4)
    ...
    def set_state(self, new_state, speed):
        ...
        if new_state not in self.animations: new_state = "idle"
...
def main():
    ...
    # --- LOAD ASSETS ---
    animations = {
        "idle":  load_gif_frames(os.path.join(BASE_CHAR_PATH, "standing.gif"), 5, ASSET_SCALE),
        "run":   load_gif_frames(os.path.join(BASE_CHAR_PATH, "running.gif"), 12, ASSET_SCALE),
        "jump":  load_gif_frames(os.path.join(BASE_CHAR_PATH, "jumping.gif"), 15, ASSET_SCALE),
        "dance": load_gif_frames(os.path.join(BASE_CHAR_PATH, "jesse_dancing.gif"), 10, ASSET_SCALE),
    }
    ...
    while running:
        ...
        if keys[pygame.K_d] and dx == 0: is_dancing = True
