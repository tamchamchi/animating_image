...
class VtracerBinarySearch(IConverter):
    ...
    def vtracer_png_to_svg(self, image, size_range=(50, 500), limit=10000):
        """
        Convert the given raster image to an SVG while preserving aspect ratio.
        A binary search is used to find the largest possible scale such that the
        compressed SVG string does not exceed the given character-length limit.

        Parameters
        ----------
        image : PIL.Image
            Input raster image to be vectorized.
        size_range : tuple(int, int)
            Minimum and maximum allowed sizes for the longest image side.
        limit : int
            Maximum allowed character length of the resulting SVG.

        Returns
        -------
        best_svg : str
            SVG string generated at the largest possible scale within the limit.
        """

        # Original image dimensions and aspect ratio
        orig_w, orig_h = image.size
        aspect = orig_w / orig_h  # width / height ratio

        lo, hi = size_range
        best_svg = None

        # -------------------------------------------------------------
        # Binary search for the largest size that keeps the SVG ≤ limit
        # -------------------------------------------------------------
        while abs(lo - hi) > 5:
            mid = (lo + hi) // 2
            ...
            # Vectorize + compress
            svg = self._convert_svg_with_vtracer(image, (new_w, new_h))
            svg = self._svg_compress(svg)
            length = len(svg)

            if length <= limit:
                # This scale is acceptable → try a larger one
                best_svg = svg
                lo = mid + 1
            else:
                # Too large → shrink the search space
                hi = mid - 1

        # -------------------------------------------------------------
        # Final fallback: if no valid size found within the search loop
        # -------------------------------------------------------------
        if best_svg is None:
            final = lo

            # Compute fallback size with preserved aspect ratio
            if orig_w >= orig_h:
                new_w = final
                new_h = int(final / aspect)
            else:
                new_h = final
                new_w = int(final * aspect)
            ...
            if len(svg) <= limit:
                ...
                # If still failing, return a minimal placeholder SVG
                best_svg = (
                    f'<svg width="{new_w}" height="{new_h}" '
                    f'xmlns="http://www.w3.org/2000/svg">'
                    '<rect width="100%" height="100%" fill="gray"/></svg>'
                )
    ...
    def convert(self, images: Image.Image, limit=10000) -> List[str]:
